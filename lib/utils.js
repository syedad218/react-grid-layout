...
/**
 * Compact an item in the layout using first-fit scanning for 'wrap' type.
 *
 * Modifies item.
 *
 */
export function compactItem(
  ...
  if (compactV) {
    ...
    // First-fit scanning: find the first available position left-to-right, top-to-bottom
    const optimalPosition = findFirstFitPosition(compareWith, l, cols);
    l.x = optimalPosition.x;
    l.y = optimalPosition.y;
    
    // Return early since we've found the optimal position
...
/**
 * Find the first available position using left-to-right, top-to-bottom scanning
 * for wrap compaction mode.
 *
 * @param {Layout} compareWith - Layout items already placed
 * @param {LayoutItem} item - Item to place
 * @param {number} cols - Number of columns in the grid
 * @return {Object} Position with x and y coordinates
 */
function findFirstFitPosition(compareWith: Layout, item: LayoutItem, cols: number): { x: number, y: number } {
  // If no items to compare with, place at top-left
  if (compareWith.length === 0) {
    return { x: 0, y: 0 };
  }
  
  // Build collision map for efficient lookups
  const collisionMap = new Set();
  let maxY = 0;
  
  compareWith.forEach(layoutItem => {
    maxY = Math.max(maxY, layoutItem.y + layoutItem.h);
    for (let y = layoutItem.y; y < layoutItem.y + layoutItem.h; y++) {
      for (let x = layoutItem.x; x < layoutItem.x + layoutItem.w; x++) {
  ...
  const isPositionFree = (x: number, y: number, w: number, h: number): boolean => {
    // Check bounds
    if (x + w > cols || x < 0 || y < 0) return false;
    
    // Check collision map
    for (let dy = 0; dy < h; dy++) {
      for (let dx = 0; dx < w; dx++) {
        if (collisionMap.has(`${x + dx},${y + dy}`)) {
          return false;
        }
      }
    }
  ...
  // Scan left-to-right, top-to-bottom for the first available position
  for (let y = 0; y <= maxY + item.h; y++) {
    for (let x = 0; x <= cols - item.w; x++) {
      if (isPositionFree(x, y, item.w, item.h)) {
        return { x, y };
      }
    }
  }
...
export function sortLayoutItems(
  ...
  if (compactType === "wrap") return sortLayoutItemsForWrap(layout);
  else return layout;
}
...
/**
 * Sort layout items for wrap compaction: prioritize items that can be compacted both horizontally and vertically.
 * Sort by row first, then by column, which allows for effective horizontal compaction within rows
 * and vertical compaction between rows.
 *
 * Does not modify Layout.
 */
export function sortLayoutItemsForWrap(layout: Layout): Layout {
  return layout.slice(0).sort(function (a, b) {
    // First sort by row (y coordinate)
    if (a.y !== b.y) {
      return a.y - b.y;
    }
    // Then sort by column (x coordinate) within the same row
    return a.x - b.x;
  });
}
